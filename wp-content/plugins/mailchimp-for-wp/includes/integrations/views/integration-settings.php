	...
	<div class="mc4wp-row">
		<div class="main-content mc4wp-col">
			...
			<div id="notice-additional-fields" class="notice notice-warning" style="display: none;">
				...
				<p><?php echo sprintf(wp_kses(__('Please ensure you <a href="%1$s">configure the plugin to send all required fields</a> or <a href="%2$s">log into your Mailchimp account</a> and make sure only the email & name fields are marked as required fields for the selected audiences.', 'mailchimp-for-wp'), array( 'a' => array( 'href' => array() ) )), 'https://www.mc4wp.com/kb/send-additional-fields-from-integrations/#utm_source=wp-plugin&utm_medium=mailchimp-for-wp&utm_campaign=integrations-page', 'https://admin.mailchimp.com/lists/'); ?></p>
			</div>
			...
			<form method="post" action="<?php echo admin_url('options.php'); ?>">
				...
				<table class="form-table">
					...
					if ($integration->has_ui_element('lists')) {
						...
						<tr valign="top">
							<th scope="row"><?php echo esc_html__('Mailchimp audiences', 'mailchimp-for-wp'); ?></th>
							<?php
							if (! empty($lists)) {
								echo '<td>';
								...
								echo esc_html__('Select the audiences to which people who check the checkbox should be subscribed.', 'mailchimp-for-wp');
								echo '</p>';
								echo '</td>';
							} else {
								echo '<td>', sprintf(wp_kses(__('No audiences found, <a href="%s">are you connected to Mailchimp</a>?', 'mailchimp-for-wp'), array( 'a' => array( 'href' => array() ) )), esc_url(admin_url('admin.php?page=mailchimp-for-wp'))), '</td>';
							}
							?>
						</tr>
						<?php
