...
if ( $need_wrap ) {
	...
	?><div id="<?php echo esc_attr($args['id']); ?>_wrap"
	...
	if ($args['type'] == 'title') {
		...
		if ( $is_preview ) {
			...
			$post_meta = trx_addons_sc_show_post_meta('sc_layouts_meta', apply_filters('trx_addons_filter_post_meta_args', array(
					...
					'class' => (int)$args['hide_overflow'] == 1 ? ' hide_overflow' : '',
					'echo' => false
...
?></div><!-- .sc_layouts_blog_item --><?php
if ( $need_wrap ) {
	?></div><!-- .sc_layouts_blog_item_wrap --><?php
}
