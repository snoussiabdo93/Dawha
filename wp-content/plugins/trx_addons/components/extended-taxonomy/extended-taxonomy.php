...
if (!function_exists('trx_addons_get_term_image')) {
	function trx_addons_get_term_image($term_id=0, $taxonomy='') {
		$image =  trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => $taxonomy, 'key' => TRX_ADDONS_EXTENDED_TAXONOMY_META_PREFIX . 'image_large' ) );
		if (empty($image)) {
			$image_id = trx_addons_get_term_meta(array('term_id' => $term_id, 'taxonomy' => '', 'key' => 'thumbnail_id'));
			if (!empty($image_id)) {
...
// Return small image (icon) from the term
if (!function_exists('trx_addons_get_term_image_small')) {
	function trx_addons_get_term_image_small($term_id=0, $taxonomy='', $icon_only=true) {
		$image =  trx_addons_get_term_meta( array( 'term_id' => $term_id, 'taxonomy' => $taxonomy, 'key' => TRX_ADDONS_EXTENDED_TAXONOMY_META_PREFIX . 'image_small' ) );
		if (empty($image) && !$icon_only) {
			$image = trx_addons_get_term_image($term_id, $taxonomy);
		}
		return trx_addons_remove_protocol($image);
	}
}
				...
				foreach($attrs as $attr) {
					...
					if (!empty($val) && !trx_addons_is_off($val)) {
						?><div class="trx_addons_extended_taxonomy_meta_row">
							<span class="trx_addons_extended_taxonomy_meta_value<?php echo $attr=='icon' ? ' ' . esc_attr($val) : ''; ?>"<?php
