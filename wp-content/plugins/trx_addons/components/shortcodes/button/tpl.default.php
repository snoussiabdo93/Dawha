	...
	if (!empty($args['image']) || !empty($args['icon'])) {
		?><span class="sc_button_icon"><?php
			if (!empty($args['image'])) {
				...
				?><img class="sc_icon_as_image" src="<?php echo esc_url($args['image']); ?>" alt="<?php esc_attr_e('Icon', 'trx_addons'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			} else if (trx_addons_is_url($args['icon'])) {
				if (strpos($args['icon'], '.svg') !== false) {
					trx_addons_show_layout(trx_addons_get_svg_from_file($args['icon']));
				} else {
					$attr = trx_addons_getimagesize($args['icon']);
					?><img class="sc_icon_as_image" src="<?php echo esc_url($args['icon']); ?>" alt="<?php esc_attr_e('Icon', 'trx_addons'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			} else {
