...
if ( $query->found_posts > 0 || count($tabs) > 0 ) {
	...
	$args['columns'] = (int)$posts_in_grid > 0 ? max(1, min(3, (int) $args['columns'])) : max(1, min(12, (int) $args['columns']));
	$args['slider'] = (int)$args['slider'] > 0 && ((int)$posts_in_grid > 0 ? $posts_count > $posts_in_grid : $posts_count > $args['columns']);
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		} else if ((int)$posts_in_grid == 0 && (int)$args['columns'] > 1 && empty($args['posts_container'])) {
