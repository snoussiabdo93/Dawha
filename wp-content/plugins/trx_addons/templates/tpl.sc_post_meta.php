...
?><div class="<?php echo esc_attr($sc); ?>_post_meta post_meta<?php echo !empty($args['class']) ? ' ' . esc_attr($args['class']) : ''; ?>"><?php
	...
	foreach ($components as $comp) {
			...
			if (!is_singular() || have_comments() || comments_open()) {
				...
				$output .= ' ' 
							. '<a href="'.esc_url(get_comments_link()).'" class="post_meta_item post_meta_comments trx_addons_icon-comment">'
								...
								. '<span class="post_meta_label">' . _n( 'Comment', 'Comments', $post_comments, 'trx_addons' ) . '</span>'
							. '</a>'
							. ' ';
			}
		...
		// Views
			...
			$output .= ' '
						. (is_singular()
							...
							. '<span class="post_meta_label">' . _n( 'View', 'Views', $post_views, 'trx_addons' ) . '</span>'
						. (is_singular()
							? '</span>'
							: '</a>'
							)
		...
		// Custom meta data
			...
			do_action('trx_addons_action_show_post_meta', $comp, $post_id);
