<div class="front_page_section front_page_section_features<?php
	$heaven11_scheme = heaven11_get_theme_option( 'front_page_features_scheme' );
	if ( ! heaven11_is_inherit( $heaven11_scheme ) ) {
		echo ' scheme_' . esc_attr( $heaven11_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( heaven11_get_theme_option( 'front_page_features_paddings' ) );
?>"
		<?php
		$heaven11_css      = '';
		$heaven11_bg_image = heaven11_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$heaven11_anchor_icon = heaven11_get_theme_option( 'front_page_features_anchor_icon' );
	$heaven11_anchor_text = heaven11_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( heaven11_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$heaven11_css      = '';
			$heaven11_bg_mask  = heaven11_get_theme_option( 'front_page_features_bg_mask' );
			$heaven11_bg_color_type = heaven11_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $heaven11_bg_color_type ) {
				$heaven11_bg_color = heaven11_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$heaven11_caption = heaven11_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $heaven11_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $heaven11_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(( $heaven11_caption ), 'heaven11_kses_content'); ?></h2>
				<?php
			}

			// Description (text)
			$heaven11_description = heaven11_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $heaven11_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $heaven11_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $heaven11_description ), 'heaven11_kses_content' ) ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
				...
				if ( ! heaven11_exists_trx_addons() ) {
					...
					heaven11_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
				}
			}
			?>
			</div>
