	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="heaven11_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.heaven11_post_editor' );
					if ( link.length > 0 ) {
						heaven11_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.heaven11_post_editor', function(e) {
					heaven11_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('heaven11_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function heaven11_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
