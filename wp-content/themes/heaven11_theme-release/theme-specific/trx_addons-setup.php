...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'heaven11_trx_addons_theme_specific_setup1' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'heaven11_trx_addons_theme_specific_components' );
	function heaven11_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'heaven11_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'heaven11_trx_addons_theme_specific_setup1', 1 );
	function heaven11_trx_addons_theme_specific_setup1() {
		if ( heaven11_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'heaven11_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'heaven11_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'heaven11_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'heaven11_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'heaven11_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'heaven11_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'heaven11_trx_addons_widgets_list' );
		}
	}
}

// CV
if ( ! function_exists( 'heaven11_trx_addons_cv_enable' ) ) {
	
	function heaven11_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return HEAVEN11_THEME_FREE
					? false     // Free version
					: false;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'heaven11_trx_addons_demo_enable' ) ) {
	
	function heaven11_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'heaven11_trx_addons_edd_themes_market_enable' ) ) {
	
	function heaven11_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'heaven11_trx_addons_api_list' ) ) {
	
	function heaven11_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'heaven11_trx_addons_cpt_list' ) ) {
	
	function heaven11_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'heaven11_trx_addons_sc_list' ) ) {
	
	function heaven11_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'heaven11_trx_addons_widgets_list' ) ) {
	
	function heaven11_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'heaven11_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'heaven11_trx_addons_menu_cache' );
	function heaven11_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'heaven11_trx_addons_localize_script' ) ) {
	...
	function heaven11_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = heaven11_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'heaven11_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'heaven11_trx_addons_sc_type', 10, 2 );
	function heaven11_trx_addons_sc_type( $list, $sc ) {
...
if ( ! function_exists( 'heaven11_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'heaven11_trx_addons_sc_prepare_atts', 10, 2 );
	function heaven11_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = heaven11_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = heaven11_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'heaven11_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'heaven11_trx_addons_sc_atts', 10, 2 );
	function heaven11_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'heaven11_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'heaven11_trx_addons_sc_map', 10, 2 );
	function heaven11_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'heaven11' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'heaven11' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( heaven11_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'heaven11' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'heaven11' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( heaven11_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'heaven11_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'heaven11_trx_addons_sc_output', 10, 4 );
	function heaven11_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! heaven11_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'heaven11_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'heaven11_trx_addons_sc_item_title_tag' );
	function heaven11_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'heaven11_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'heaven11_trx_addons_sc_item_button_args', 10, 3 );
	function heaven11_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'heaven11_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'heaven11_trx_addons_slider_title', 10, 2 );
	function heaven11_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'heaven11_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'heaven11_trx_addons_sc_googlemap_styles' );
	function heaven11_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'heaven11' );
		return $list;
	}
}
...
if ( ! function_exists( 'heaven11_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'heaven11_trx_addons_tiny_mce_style_formats' );
	function heaven11_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'heaven11' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'heaven11' ),
						'selector' => 'ul',
				...
				if ( esc_html__( 'List styles', 'heaven11' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Alternative', 'heaven11' ),
						'selector' => 'ul',
						'classes'  => 'trx_addons_list trx_addons_list_alter',
					);
				}
				if ( esc_html__( 'List styles', 'heaven11' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						...
						'selector' => 'ul',
						'classes'  => 'trx_addons_list trx_addons_list_extra',
					);
				}
				if ( esc_html__( 'List styles', 'heaven11' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Extra with icon', 'heaven11' ),
						'selector' => 'ul',
						'classes'  => 'trx_addons_list trx_addons_list_extra-icon',
					);
				}
				if ( $v['title'] == esc_html__('Inline', 'heaven11') ) {
					$style_formats[$k]['items'][] = array(
						'title' => esc_html__('Accent hovered', 'heaven11'),
						'inline' => 'span',
...
if ( ! function_exists( 'heaven11_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'heaven11_filter_allow_override_header_image', 'heaven11_trx_addons_allow_override_header_image' );
	function heaven11_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					heaven11_is_team_page()
					|| heaven11_is_cars_page()
					|| heaven11_is_cars_agents_page()
					|| heaven11_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'heaven11_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'heaven11_trx_addons_thumb_size', 10, 2 );
	function heaven11_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
if ( ! function_exists( 'heaven11_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'heaven11_trx_addons_meta_box_fields', 10, 2 );
	function heaven11_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'heaven11' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'heaven11' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
		...
		if ( defined( 'TRX_ADDONS_CPT_PROPERTIES_PT' ) && TRX_ADDONS_CPT_PROPERTIES_PT == $post_type ) {
			if ( ! isset( $mb['inverse_image_section'] ) ) {
				$mb['inverse_image_section'] = array(
					'title' => esc_html__('Inverse image', 'heaven11'),
					'desc' => wp_kses_data(__('Inverse image for Alternative properties type', 'heaven11')),
					'type' => "section",
				);
				}
			if ( ! isset( $mb['inverse_image'] ) ) {
				$mb['inverse_image'] = array(
					'title'   => esc_html__( 'Inverse image', 'heaven11' ),
					'desc'    => wp_kses_data( __( "Inverse image for Alternative properties type", 'heaven11' ) ),
					'std'     => '',
					'details' => true,
...
// Remove TRX Addons
if( !function_exists('remove_parent_filters_trx_options') ) {
	add_filter( 'trx_addons_filter_options', 'remove_parent_filters_trx_options', 10, 1 );
	function remove_parent_filters_trx_options($list){
...
if( !function_exists('remove_parent_filter_layout_types') ) {
	add_filter( 'trx_addons_filter_layout_types', 'remove_parent_filter_layout_types', 10, 1 );
	function remove_parent_filter_layout_types($list){
		unset($list['blog']);
		return $list;
	}
}