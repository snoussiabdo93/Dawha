	...
	?><div class="trx_addons_audio_wrap<?php echo esc_attr( $wrap_class ); ?>">
		...
		foreach ( $media as $item ) {
			...
			?>
				...
				if ( ! empty( $item['cover'] ) ) {
				?>
				<div class="img-audio"><img src="<?php echo esc_url( $item['cover'] ); ?>" alt="<?php esc_attr_e( 'Audio image', 'heaven11' ); ?>"></div>
				<?php } ?>
					...
					if ( ! empty( $item['author'] ) || ! empty( $item['caption'] ) ) {
						...
						<div class="audio_info">
							...
							if ( '#' !== $now_text && count( $media ) > 1 ) {
								echo '<h5 class="audio_now_playing">' . ( ! empty( $now_text ) ? esc_html( $now_text ) : esc_html__( 'Now Playing', 'heaven11' ) ) . ': </h5>';
							}
							if ( ! empty( $item['cover'] ) ) {
								?>
								<div class="audio_img_for_post"><?php echo esc_url( $item['cover'] ); ?></div>
							<?php }
