...
<ul class="properties_compare_list<# if (list=='') print(' properties_compare_list_empty'); #>"><#
	if (list != '') {
		_.each(list, function(v, k) {
			#><li data-property-id="<# print(k.replace('id_', '')); #>" title="<?php esc_attr_e('Click to remove this property from the compare list', 'heaven11'); ?>">{{{ v }}}</li><#
		});
	}
#></ul>

<div class="properties_compare_message"><?php esc_html_e('Select 2+ properties to compare', 'heaven11'); ?></div>

<a class="properties_compare_button sc_button" href="<?php echo esc_url(trx_addons_add_to_url(get_post_type_archive_link(TRX_ADDONS_CPT_PROPERTIES_PT), array('compare'=>1))); ?>"><?php esc_html_e('Compare', 'heaven11'); ?></a>
