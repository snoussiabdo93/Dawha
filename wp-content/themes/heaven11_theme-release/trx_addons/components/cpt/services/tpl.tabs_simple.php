		...
		?><div class="sc_services_content sc_item_content sc_item_posts_container<?php if (!empty($args['no_links'])) echo ' no_links'; ?>"><?php
			...
			while ( $query->have_posts() ) { $query->the_post();
				...
				$tabs_item = '<div class="sc_services_item sc_services_tabs_simple_item sc_services_tabs_list_item' 
								...
								. '">';
				$tabs_item .= '<div class="inner_services_tabs_simple">';
					...
					$tabs_item .= '<span id="'.esc_attr($args['id'].'_'.trim($meta['icon']).'_'.trim($trx_addons_number)).'"'
									...
									. '>'
										. (!empty($svg) 
											...
											: (!empty($img)
												? '<img class="sc_icon_as_image" src="'.esc_url($img).'" alt="' . esc_attr__('Icon', 'heaven11') . '">'
												: '')
											)
									. '</span>';
				} else if ($args['featured']=='pictogram' && !empty($meta['image'])) {
					$attr = trx_addons_getimagesize($meta['image']);
					$tabs_item .= '<span class="sc_services_item_pictogram"><img src="' . esc_url($meta['image']) . '" alt="' . esc_attr__('Icon', 'heaven11') . '"'
				...
				$tabs_item .= '<div class="sc_services_item_info">'
								. '<h6 class="sc_services_item_title">' . wp_kses(( get_the_title() ), 'heaven11_kses_content') . '</h6>'
				...
				$thumb_url_array = wp_get_attachment_image_src($thumb_id, 'heaven11-thumb-services-tab', true);
				...
				$tabs_list[] = array(
									...
									'content' => '<div class="sc_services_tabs_content_item'
													...
													. '">'
										. '<div class="sc_services_tabs_content_image">'
										. '<img class="sc_image" src="'.esc_url($thumb_url).'" alt="' . esc_attr__('Image', 'heaven11') . '">'
										. '</div>'
										. '<div class="sc_services_tabs_content_inner">'
										                . '<h4 class="sc_services_tabs_content_item_title">'
															. get_the_title()
														. '</h4>'
														...
														. (!empty($link) && !empty($args['more_text'])
															? '<div class="sc_services_item_button sc_item_button">'
																. '<a href="'.esc_url($link).'"'.(!empty($meta['link']) ? ' target="_blank"' : '').' class="'.esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_size_large', 'sc_services', $args)).'">'.esc_html($args['more_text']).'</a>'
																. '</div>'
															: '')
										. '</div>'
													. '</div>'
